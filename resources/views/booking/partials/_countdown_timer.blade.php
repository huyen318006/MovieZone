{{-- ============================================================
     BOOKING COUNTDOWN TIMER - Component tÃ¡i sá»­ dá»¥ng
     Include vÃ o báº¥t ká»³ trang nÃ o cáº§n hiá»ƒn thá»‹ bá»™ Ä‘áº¿m ngÆ°á»£c:
     @include('booking._countdown_timer', ['secondsLeft' => $secondsLeft])
     ============================================================ --}}

{{-- Thanh countdown ná»•i sticky á»Ÿ Ä‘áº§u trang --}}
    ...
    <div class="countdown-inner">
        <div class="countdown-left">
            ...
            <div class="countdown-label">
                <span class="countdown-label-text">Thá»i gian giá»¯ gháº¿</span>
...
<div class="countdown-expired-overlay" id="countdownExpiredOverlay">
    <div class="countdown-expired-modal">
        ...
        <h3>Háº¿t thá»i gian giá»¯ gháº¿!</h3>
        <p>PhiÃªn giá»¯ gháº¿ 5 phÃºt Ä‘Ã£ háº¿t. Gháº¿ cá»§a báº¡n Ä‘Ã£ Ä‘Æ°á»£c giáº£i phÃ³ng cho khÃ¡ch khÃ¡c.</p>
        <div class="expired-countdown-redirect">
            <div class="expired-spinner"></div>
            <span>Äang chuyá»ƒn vá» chá»n gháº¿... (<span id="redirectCountdown">3</span>s)</span>
        </div>
        @php
        ...
        <a href="{{ $resolvedShowtimeId
            ...
            <i class="fa-solid fa-arrow-left"></i> Chá»n gháº¿ láº¡i
        </a>
    </div>
</div>
...
(function() {
    ...
    // Láº¥y showtime_id tá»« URL hoáº·c session Ä‘á»ƒ build redirect URL
    const seatPageUrl = "{{ $resolvedShowtimeId
        ? \App\Helpers\TabAuthHelper::route('booking.seat', ['showtime_id' => $resolvedShowtimeId])
        : \App\Helpers\TabAuthHelper::route('showtimes') }}";

    /**
     * TÃ­nh thá»i gian cÃ²n láº¡i dá»±a trÃªn server timestamp + clock offset.
    ...
    // Hiá»ƒn thá»‹ ngay khi load
    updateDisplay();
    // Náº¿u Ä‘Ã£ háº¿t háº¡n ngay tá»« Ä‘áº§u
    if (getSecondsLeft() <= 0) {
        clearInterval(interval);
        expired = true;
        showExpiredModal();
    ...
    /**
     * VISIBILITY CHANGE: Khi user quay láº¡i tab sau khi tab bá»‹ background,
     * tÃ­nh láº¡i thá»i gian ngay láº­p tá»©c (khÃ´ng chá» setInterval 1 giÃ¢y tiáº¿p theo).
     */
    ...
    function showExpiredModal() {
        ...
        // PhÃ¡t event Ä‘á»ƒ trang payment biáº¿t countdown Ä‘Ã£ háº¿t
        window.dispatchEvent(new CustomEvent('countdownExpired'));

        // Äáº¿m ngÆ°á»£c redirect 3 giÃ¢y
