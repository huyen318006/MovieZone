...
@section('content')
            ...
            <div class="detail-showtime-grid">
                @foreach ($movie->showtimes as $showtime)
                    <article class="detail-showtime-card">
                        <div>
                            <span
                                class="showtime-date">{{ $showtime->start_time ? \Carbon\Carbon::parse($showtime->start_time)->format('d/m/Y') : 'Đang cập nhật' }}</span>
    ...
    <section class="movie-detail-section movie-review-section">
        ...
        <div class="my-review-container mb-5">
            ...
            @if ($cUser)
                    ...
                    <div class="my-review-box" id="review-display-{{ $myReview->id }}" data-aos="fade-up">
                            ...
                            <div class="review-user-info">
                                ...
                                <div>
                                    ...
                                    <small class="text-muted">Gửi lúc:
                                        {{ $myReview->created_at?->format('H:i d/m/Y') }}</small>
                                </div>
                            </div>
                        ...
                        <div class="review-box-content mt-3">
                            <div class="stars-display mb-2">
                                @for ($i = 1; $i <= 5; $i++)
                                    <i
                                        class="fa-solid fa-star {{ $i <= $myReview->rating ? 'text-warning' : 'text-secondary' }}"></i>
                                @endfor
                                <span class="rating-number ms-2">{{ $myReview->rating }}.0 / 5.0</span>
                            </div>
                            <p class="review-comment">
                                {{ $myReview->comment ?: 'Bạn chỉ chấm điểm sao, không để lại bình luận.' }}</p>
                        </div>
                    </div>

                    <!-- Form chỉnh sửa ẩn đi lúc đầu (A2) -->
                    <div class="review-form-box" id="review-edit-form-{{ $myReview->id }}" style="display: none;"
                        data-aos="fade-up">
                        ...
                        <form action="{{ \App\Helpers\TabAuthHelper::route('reviews.update', $myReview->id) }}" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="star-rating edit-star-rating">
                                    @for ($i = 5; $i >= 1; $i--)
                                        <input type="radio" id="edit-star{{ $i }}" name="rating"
                                            value="{{ $i }}" {{ $myReview->rating == $i ? 'checked' : '' }} />
                                        <label for="edit-star{{ $i }}" title="{{ $i }} sao"><i
                            ...
                            <div class="mb-4">
                                <label for="edit-comment" class="form-label font-weight-bold text-light">Bình luận của
                                    bạn:</label>
                                <textarea class="form-control bg-dark text-white border-secondary" id="edit-comment" name="comment" rows="4"
                                    maxlength="500" placeholder="Viết bình luận của bạn...">{{ old('comment', $myReview->comment) }}</textarea>
                                @error('comment')
                            ...
                            <div class="form-actions d-flex gap-2">
                                ...
                                <button type="button" class="btn-cancel-review"
                                    onclick="hideEditForm({{ $myReview->id }})">Hủy</button>
                            </div>
                        </form>
                    </div>
                @elseif($isAdminUser)
                    <div class="alert alert-info bg-dark border-secondary text-info d-flex align-items-center gap-3 py-3 px-4 rounded-4"
                        data-aos="fade-up">
                ...
                @elseif(!$hasBooking)
                    ...
                    <div class="alert alert-warning bg-dark border-secondary text-warning d-flex align-items-center gap-3 py-3 px-4 rounded-4"
                        data-aos="fade-up">
                        ...
                        <div>
                            ...
                            Chỉ các khách hàng có vé đã thanh toán (PAID) hoặc đã check-in đối với phim này mới có thể viết đánh
                            giá.
                        </div>
                    </div>
                @else
                            ...
                            <div class="mb-4">
                                ...
                                <div class="star-rating">
                                    <input type="radio" id="star5" name="rating" value="5" /><label
                                        for="star5" title="5 sao"><i class="fa-solid fa-star"></i></label>
                                    <input type="radio" id="star4" name="rating" value="4" /><label
                                        for="star4" title="4 sao"><i class="fa-solid fa-star"></i></label>
                                    <input type="radio" id="star3" name="rating" value="3" /><label
                                        for="star3" title="3 sao"><i class="fa-solid fa-star"></i></label>
                                    <input type="radio" id="star2" name="rating" value="2" /><label
                                        for="star2" title="2 sao"><i class="fa-solid fa-star"></i></label>
                                    <input type="radio" id="star1" name="rating" value="1" /><label
                                        for="star1" title="1 sao"><i class="fa-solid fa-star"></i></label>
                                </div>
                            </div>

                            <div class="mb-4">
                                <label for="comment" class="form-label font-weight-bold text-light">Bình luận (không bắt
                                    buộc, tối đa 500 ký tự):</label>
                                <textarea class="form-control bg-dark text-white border-secondary" id="comment" name="comment" rows="4"
                                    placeholder="Nhập suy nghĩ của bạn về phim (tùy chọn)..." maxlength="500">{{ old('comment') }}</textarea>
                                @error('comment')
                                    <div class="text-danger mt-2" style="font-size: 14px;">
                                        <i class="fa-solid fa-circle-exclamation me-1"></i> {{ $message }}
                                    </div>
            ...
            @else
                ...
                <div class="alert alert-warning bg-dark border-secondary text-warning d-flex align-items-center gap-3 py-3 px-4 rounded-4"
                    ...
                    <div>
                        ...
                        Vui lòng <a href="{{ route('login') }}"
                            class="text-warning text-decoration-underline font-weight-bold">đăng nhập</a> và mua vé hợp lệ để
                        đánh giá phim này.
                    </div>
                </div>
            @endif
        ...
        <div class="public-reviews-container">
            <h3 class="mb-4 text-light"><i class="fa-regular fa-comments text-primary me-2"></i> Nhận xét từ người xem
                khác</h3>
                                        ...
                                        @if (\App\Models\UserRole::where('user_id', $cUser->id)->where('role_id', 1)->exists())
                                            ...
                                            <form action="{{ \App\Helpers\TabAuthHelper::route('reviews.destroy', $review->id) }}" method="POST"
                                                ...
                                                <button type="submit"
                                                    ...
                                                    style="font-size: 13px;" title="Xóa cứng (Admin)">
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content trailer-modal-content">
                ...
                <div class="modal-body">
                    @if ($trailerEmbedUrl)
                        <iframe src="{{ $trailerEmbedUrl }}" title="Trailer {{ $movie->title }}"
